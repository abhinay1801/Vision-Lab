...
const Help_RealisticCylinderBowlExperiment = () => {
  ...
  const [solidInputs, setSolidInputs] = useState({
    Wood: 0,
    Iron: 0,
    Plastic: 0,
  });
    ...
    if (isPouring && pourProgress < parseFloat(liquidAmount)) {
      const interval = setInterval(() => {
        setPourProgress((prev) => {
          const currentTotal = Object.values(liquids).reduce(
            (sum, val) => sum + val,
            0
          );
          const remaining = 100 - currentTotal;
          const pourStep =
            Math.min(1, remaining, parseFloat(liquidAmount) - prev) / 2;
          if (
            prev + pourStep >= parseFloat(liquidAmount) ||
            currentTotal + pourStep >= 100
          ) {
            setIsPouring(false);
            setPourProgress(0);
          }
        ...
        setSolids((prev) =>
          prev.map((solid) => {
            ...
            return {
              ...
              bounce:
                Math.abs(targetY - newY) < 5 ? (solid.bounce || 0) + 0.1 : 0,
            };
          })
        );
      }, 50);
  ...
  const getLiquidLayers = () => {
    const totalPercent = Object.values(liquids).reduce(
      (sum, val) => sum + val,
      0
    );
  ...
  const addLiquid = () => {
    ...
    if (newAmount > remainingCapacity) {
      setError(
        `Cannot exceed 100% capacity. Only ${remainingCapacity.toFixed(
          1
        )}% remains.`
      );
      return;
        ...
        {showHelp && (
          <div className="bg-white/90 backdrop-blur-sm p-6 rounded-xl shadow-lg mb-6 animate-fade-in">
            ...
            <ul className="list-disc pl-5 space-y-2 text-gray-700">
              ...
              <li>
                Watch realistic physics simulation with waves and buoyancy
              </li>
            </ul>
          </div>
        )}
            ...
            <div className="relative bg-white/95 backdrop-blur-md shadow-2xl rounded-xl p-6 w-full border border-blue-300">
              <div className="text-center mb-4">
                ...
                <p className="text-gray-600">
                  Observe liquid layers and buoyancy
                </p>
              </div>
                  ...
                  {solids.map((solid) => (
                    <div
                      ...
                      className={`absolute w-12 h-12 ${
                        SOLIDS[solid.type].color
                      } rounded-lg shadow-xl transition-all duration-50 flex items-center justify-center`}
                      style={{
                        left: `${solid.x}px`,
                        bottom: `${solid.y + Math.sin(solid.bounce) * 5}px`,
                        transform: `rotate(${
                          solid.rotation
                        }deg) scale(1.${Math.round(solid.bounce)})`,
                ...
                {Object.entries(SOLIDS).map(([solid, props]) => (
                  <div key={solid} className="bg-blue-50/50 p-3 rounded-lg">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleSolidAmountChange(solid, e.target.value)
                      }
...
export default Help_RealisticCylinderBowlExperiment;
